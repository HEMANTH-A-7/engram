...
Pure Python: no LLM calls, no Ollama, no Cognee. Each test gets an isolated
SQLite file under pytest's tmp_path, so this runs in the fast default suite
(no `integration` marker) and proves the versioning logic is correct on its
own, independent of extraction quality.
"""
...
def test_first_write_has_no_supersede(tmp_path):
    _fresh(tmp_path)
    result = resolver.write("Alice", "lives_in", "Boston", "Alice lives in Boston.")
...
def test_conflicting_write_supersedes_old_fact(tmp_path):
    _fresh(tmp_path)
    first = resolver.write("Alice", "lives_in", "Boston", "Alice lives in Boston.")
...
def test_current_facts_only_returns_the_latest_version(tmp_path):
    _fresh(tmp_path)
    resolver.write("Alice", "lives_in", "Boston", "Alice lives in Boston.")
    resolver.write("Alice", "lives_in", "Seattle", "Alice moved to Seattle.")
    resolver.write("Bob", "favorite_color", "green", "Bob's favorite color is green.")
...
def test_restating_the_same_fact_is_a_no_op(tmp_path):
    _fresh(tmp_path)
    first = resolver.write("Alice", "lives_in", "Boston", "Alice lives in Boston.")
    again = resolver.write("Alice", "lives_in", "boston", "Alice still lives in Boston.")
...
def test_history_preserves_all_versions_oldest_first(tmp_path):
    _fresh(tmp_path)
    resolver.write("Alice", "lives_in", "Boston", "Alice lives in Boston.")
    resolver.write("Alice", "lives_in", "Seattle", "Alice moved to Seattle.")
    resolver.write("Alice", "lives_in", "Denver", "Alice moved to Denver.")
...
def test_different_relations_do_not_conflict(tmp_path):
    _fresh(tmp_path)
    resolver.write("Alice", "lives_in", "Boston", "Alice lives in Boston.")
    result = resolver.write("Alice", "favorite_color", "blue", "Alice's favorite color is blue.")
...
def test_datasets_are_isolated(tmp_path):
    _fresh(tmp_path)
    resolver.write("Alice", "lives_in", "Boston", "Alice lives in Boston.", dataset="a")
    resolver.write("Alice", "lives_in", "Seattle", "Alice moved to Seattle.", dataset="b")

    assert [f.object for f in resolver.current_facts("a")] == ["Boston"]
    assert [f.object for f in resolver.current_facts("b")] == ["Seattle"]


def test_reset_scoped_to_dataset(tmp_path):
    _fresh(tmp_path)
    resolver.write("Alice", "lives_in", "Boston", "Alice lives in Boston.", dataset="a")
    resolver.write("Bob", "lives_in", "Denver", "Bob lives in Denver.", dataset="b")
